...
### 474294 - omitted unconfirmed parent (2017)

Transaction 110 spends `b11a78c6c61af1cb37586f639050d74b95c2b0fd525623b6cb6a4bb4fba46a0e:1`, and that parent is not in the block.
The parent (and the child) confirmed in the canonical competing block at the same height, `000000000000000000db2504327e272fe7658fac0dd0741f46b212256e500886`.
At the candidate's parent tip the spent output did not exist, so `ConnectBlock` fails with `bad-txns-inputs-missingorspent`.
This is not an in-block ordering error: unlike 477115 and 809478, no later transaction in the body creates the output.
The violation is re-derivable from the [preserved full block](../blocks/474294-00000000000000000182acdf5657c93a0769dc6f9004047496b2e15efc6a4232.bin) together with the authenticated parent transaction and its current canonical confirmation height.
CI does not replay historical `ConnectBlock` against a reconstructed UTXO set.
...
The pastebin dump linked from that thread is a transaction-index listing of 477115, not of this block.
