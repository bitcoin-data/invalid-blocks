...
| `parent_kind` | string | Chain status of the previous block: `canonical`, `stale`, or `invalid`. `invalid` means the previous block is in this dataset. Generally descriptive; `already_confirmed_in_parent` requires `canonical` and checks the canonical-height lookup. |
...
It fetches `/block/{prev_hash}/txids` as a nonempty JSON array of lowercase 64-hex transaction IDs, rejecting duplicates.
